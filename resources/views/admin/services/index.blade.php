...
<div class="modal-body text-center">
    <img id="v_image" class="img-fluid rounded mb-3" style="max-heigh:200px">
...
document.querySelectorAll('.editServiceBtn').forEach(btn=>{
    btn.onclick = ()=>{
        serviceForm.action = `/admin/services/${btn.dataset.id}`;
        form_method.value = "PUT";

        name.value = btn.dataset.name;
        room_id.value = btn.dataset.room;
        type.value = btn.dataset.type;
        icon.value = btn.dataset.icon;
        description.value = btn.dataset.description;

        preview_image.innerHTML = btn.dataset.image
            ? `<img src="/storage/${btn.dataset.image}" class="img-fluid rounded" width="80">`
            : '';
    }
});

function viewService(s){
    v_image.src = s.image ? '/storage/'+s.image : '';
    v_name.innerText = s.name;
    v_room.innerText = s.room?.name ?? '-';
    v_type.innerText = s.type;
    v_icon.className = s.icon;
    v_description.innerText = s.description ?? 'N/A';

    new bootstrap.Modal(viewServiceModal).show();
}
</script>
